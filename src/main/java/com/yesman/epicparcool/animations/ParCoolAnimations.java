...
@EventBusSubscriber(modid = EpicParCool.MODID, bus = EventBusSubscriber.Bus.MOD)
				...
				new StaticAnimation(false, accessor, Armatures.BIPED)
						...
						.addProperty(StaticAnimationProperty.ELAPSED_TIME_MODIFIER, (self, entitypatch, speed, prevElapsedTime, elapsedTime) -> {
							...
							if (entitypatch instanceof PlayerPatch<?> playerpatch) {
								...
								if (parkourability == null) return Pair.of(prevElapsedTime, modular);

								RideZipline action = parkourability.get(RideZipline.class);
								if (action == null) return Pair.of(prevElapsedTime, modular);

								if (!(action instanceof ParCoolMixinRideZiplineAccessor zi)) {
									return Pair.of(prevElapsedTime, modular);
								}
								double d1 = Math.abs(zi.getSpeed());

								Vec3 offset = action.getEndOffsetFromStart();
								if (offset == null) return Pair.of(prevElapsedTime, modular);
								offset = offset.normalize();

								Vec3 lookVec = VectorUtil.fromYawDegree(player.getYRot());
				...
				new StaticAnimation(false, accessor, Armatures.BIPED)
						...
						.addProperty(StaticAnimationProperty.ELAPSED_TIME_MODIFIER, (self, entitypatch, speed, prevElapsedTime, elapsedTime) -> {
							...
							if (entitypatch instanceof PlayerPatch<?> playerpatch) {
								...
								RideZipline rideZipline = parkourability.get(RideZipline.class);
								if (rideZipline == null) return Pair.of(prevElapsedTime, modular);

								if (!(rideZipline instanceof ParCoolMixinRideZiplineAccessor ziplineAccessor)) {
									return Pair.of(prevElapsedTime, modular);
								}
								double d1 = Math.abs(ziplineAccessor.getSpeed());
								double d2 = -1.0D / (1.0D * d1 + 1.0D) + 1.0D;

								Vec3 offset = rideZipline.getEndOffsetFromStart();
								if (offset == null) return Pair.of(prevElapsedTime, modular);

								double yawToEndPoint = VectorUtil.toYaw(offset);
								double playerYaw = Mth.wrapDegrees(player.getYRot() - yawToEndPoint);
